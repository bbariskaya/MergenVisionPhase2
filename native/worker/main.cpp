...
int main(int argc, char* argv[]) {
    ...
    std::string tracker_config = (argc >= 5) ? argv[4] : "/app/native/configs/tracker_NvDCF_mergen.yml";
    if (g_access(tracker_config.c_str(), F_OK) != 0 && tracker_config[0] != '/') {
        tracker_config = std::string("native/configs/") + tracker_config;
    }
    ...
    std::string preprocess_config = "/app/native/configs/retinaface_preprocess.txt";
    if (g_access(preprocess_config.c_str(), F_OK) != 0) {
        preprocess_config = "native/configs/retinaface_preprocess.txt";
    }
    ...
    std::string engine_path = "/app/engines/retinaface_r50_dynamic.bs1.opt64.max256.fp16.trt1014.engine";
    if (g_access(engine_path.c_str(), F_OK) != 0) {
        engine_path = "engines/retinaface_r50_dynamic.bs1.opt64.max256.fp16.trt1014.engine";
    }
    ...
    if (ctx.manifest_f) {
        ...
        fprintf(ctx.manifest_f, "  \"preprocess_config\": \"native/configs/retinaface_preprocess.txt\",\n");
