...
INPUT_PATH = "/app/backend/artifacts/videos/friendsshort_50f.mp4"
...
def _dockerize(entrypoint: str, *args: str, extra_env: dict | None = None) -> list[str]:
    env = {"CUDA_VISIBLE_DEVICES": "0", "GST_PLUGIN_PATH": "/app/backend/native/build"}
...
def run_worker_under_nsys(output_basename: str) -> int:
    run_dir = REPO / "backend" / "out" / f"{output_basename}_run"
    run_dir.mkdir(parents=True, exist_ok=True)
    rep_path = f"/app/backend/out/{output_basename}.nsys-rep"
    sqlite_path = f"/app/backend/out/{output_basename}.sqlite"

    profile_cmd = _dockerize(
        "/usr/local/cuda/bin/nsys",
        "profile",
        "-t", "cuda,nvtx",
        "-o", f"/app/backend/out/{output_basename}",
        "--force-overwrite", "true",
        WORKER,
        INPUT_PATH,
        f"/app/backend/out/{output_basename}_run",
        "0",
    )
    ...
    return worker_exit, Path(REPO / "backend" / "out" / f"{output_basename}.sqlite")
...
def test_no_full_output_tensor_d2h_and_clean_exit():
    ...
    proc = _run(
        ["python3", str(REPO / "backend" / "scripts" / "sanity_check_detections.py"),
         str(REPO / "backend" / "out" / "hotpath_contract_run" / "detections.jsonl")],
        check=False,
    )
