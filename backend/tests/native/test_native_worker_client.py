...
class NativeDetectorClientTests(unittest.TestCase):
    ...
    def test_default_container_and_worker(self) -> None:
        self.assertEqual(self.client.container, "nvcr.io/nvidia/deepstream:9.0-triton-multiarch")
        self.assertEqual(self.client.worker_path, "/app/backend/native/build/deepstream_face_worker")
    ...
    def test_run_command_uses_container_paths(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            cmd = client.run_command("backend/artifacts/videos/clip.mp4", "backend/out/cli")
            self.assertIn("/app/backend/native/build/deepstream_face_worker", cmd)
